...
def _current_expected_place():
    ...
    if _global_expected_place_ is None:
        ...
        elif core.is_compiled_with_sw():
            _global_expected_place_ = core.SWPlace()
        else:
            _global_expected_place_ = core.CPUPlace()
...
def _sw_num():
    if "SW_NUM" not in os.environ.keys():
        if multiprocessing.cpu_count() > 1:
            sys.stderr.write(
                '!!! The CPU_NUM is not specified, you should set CPU_NUM in the environment variable list.\n'
                'CPU_NUM indicates that how many CPUPlace are used in the current task.\n'
                ...
                '!!! The default number of CPU_NUM=1.\n'.format(
                    multiprocessing.cpu_count(), multiprocessing.cpu_count()))
        os.environ['SW_NUM'] = str(1)
    sw_num = os.environ.get('SW_NUM')
    return int(sw_num)
...
def is_compiled_with_sw():
    return core.is_compiled_with_sw()
...
def sw_places(device_count=None):
    if device_count is None:
        device_count = _sw_num()
    return [core.SWPlace()] * device_count
...
def _get_paddle_place(place):
    ...
    if place is None:
        return place
    if isinstance(place, (core.Place, core.XPUPlace, core.CPUPlace,core.SWPlace,
