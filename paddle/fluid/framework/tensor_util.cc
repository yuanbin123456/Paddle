...
void TensorCopy(const Tensor& src, const platform::Place& dst_place,
  ...
  else if (platform::is_sw_place(src_place) &&  // NOLINT
           platform::is_cpu_place(dst_place)) {
    memory::Copy(BOOST_GET_CONST(platform::CPUPlace, dst_place), dst_ptr,
                 BOOST_GET_CONST(platform::SWPlace, src_place), src_ptr, size);
  } else if (platform::is_cpu_place(src_place) &&
             platform::is_sw_place(dst_place)) {
    memory::Copy(BOOST_GET_CONST(platform::SWPlace, dst_place), dst_ptr,
                 BOOST_GET_CONST(platform::CPUPlace, src_place), src_ptr, size);
  } else if (platform::is_sw_place(src_place) &&
             platform::is_sw_place(dst_place)) {
    if (src_ptr == dst_ptr) {
      VLOG(3) << "Skip copy the same data async from " << src_place << " to "
              << dst_place;
...
class AnyVisitor : public boost::static_visitor<bool> {
 ...
 public:
  ...
  bool GetResult(const framework::Tensor& out,
                 const platform::SWPlace& sw) const {
    return GetResultHelper(out, sw);
  }
...
struct BothFalseVisitor : public boost::static_visitor<> {
  ...
  void VisitorImpl(const platform::SWPlace& cpu) const {
    int num = in_.numel();
    const bool* in_ptr = in_.data<bool>();
    bool* out_ptr = out_->data<bool>();
    for (int i = 0; i < num; ++i) {
