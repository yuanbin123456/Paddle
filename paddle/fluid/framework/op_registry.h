...
#define REGISTER_OP_SW_KERNEL(op_type, ...) \
  REGISTER_OP_KERNEL(op_type, SW, ::paddle::platform::SWPlace, __VA_ARGS__)
...
#define REGISTER_OP_SW_KERNEL_FUNCTOR(op_type, ...)                  \
  REGISTER_OP_KERNEL_EX(                                              \
      op_type, SW, ::paddle::platform::SWPlace, DEFAULT_TYPE,       \
      ::paddle::framework::OpKernelType::kDefaultCustomizedTypeValue, \
      __VA_ARGS__)
...
#define USE_SW_ONLY_OP(op_type) \
  USE_OP_ITSELF(op_type);        \
  USE_OP_DEVICE_KERNEL(op_type, SW);
