...
#endif
    } else if (platform::is_sw_place(place_)) {
#ifdef PADDLE_WITH_SW
      gc.reset(new SWGarbageCollector(
          BOOST_GET_CONST(platform::SWPlace, place_), max_memory_size));
#else
      PADDLE_THROW(
          platform::errors::Unimplemented("No SW gc found in CPU/GPU paddle"));
#endif
