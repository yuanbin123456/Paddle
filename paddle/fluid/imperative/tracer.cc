...
#if defined(PADDLE_WITH_SW)
      gc.reset(new framework::SWGarbageCollector(
          BOOST_GET_CONST(platform::SWPlace, place), 0));
      VLOG(10) << "Created GarbageCollector at " << place;
#else
      PADDLE_THROW(platform::errors::PermissionDenied(
          "Paddle can't use SW device since it's not compiled with SW,"
