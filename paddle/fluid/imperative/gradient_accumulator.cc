...
class TensorAddFunctor : public boost::static_visitor<> {
 public:
  ...
  void operator()(const platform::SWPlace& place) {
    platform::SWDeviceContext* ctx = dynamic_cast<platform::SWDeviceContext*>(
        platform::DeviceContextPool::Instance().Get(place));
    auto blas = operators::math::GetBlas<platform::SWDeviceContext, T>(*ctx);
    blas.AXPY(numel_, 1., x_, y_);
  }
#else
  void operator()(const platform::SWPlace& place) {
