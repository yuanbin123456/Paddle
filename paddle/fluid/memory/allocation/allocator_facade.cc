...
#include "paddle/fluid/memory/allocation/sw_allocator.h"
#endif
...
#ifdef PADDLE_WITH_SW
    system_allocators_[platform::SWPlace()] = std::make_shared<SWAllocator>();
#endif
#ifdef PADDLE_WITH_XPU
...
#ifdef PADDLE_WITH_SW
  void InitNaiveBestFitSWAllocator() {
    allocators_[platform::SWPlace()] =
        std::make_shared<NaiveBestFitAllocator>(platform::SWPlace());
  }
#endif
#if defined(PADDLE_WITH_CUDA) || defined(PADDLE_WITH_HIP)
...
#ifdef PADDLE_WITH_SW
    places.emplace_back(platform::SWPlace());
#endif
#if defined(PADDLE_WITH_CUDA) || defined(PADDLE_WITH_HIP)
    int device_count = platform::GetCUDADeviceCount();
