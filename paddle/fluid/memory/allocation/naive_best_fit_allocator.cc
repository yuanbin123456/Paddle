...
struct Usage : public boost::static_visitor<size_t> {
  ...
  size_t operator()(const platform::SWPlace &cpu) const;
};
...
BuddyAllocator *GetSWBuddyAllocator() {
  // We tried thread_local for inference::RNN1 model, but that not works much
  // for multi-thread test.
  static std::once_flag init_flag;
  static detail::BuddyAllocator *a = nullptr;
...
template <>
void *Alloc<platform::SWPlace>(const platform::SWPlace &place, size_t size) {
  VLOG(10) << "Allocate " << size << " bytes on " << platform::Place(place);
  void *p = GetSWBuddyAllocator()->Alloc(size);
...
template <>
void Free<platform::SWPlace>(const platform::SWPlace &place, void *p,
                              size_t size) {
  VLOG(10) << "Free pointer=" << p << " on " << platform::Place(place);
  GetSWBuddyAllocator()->Free(p);
}

template <>
uint64_t Release<platform::SWPlace>(const platform::SWPlace &place) {
  return GetSWBuddyAllocator()->Release();
}

template <>
size_t Used<platform::SWPlace>(const platform::SWPlace &place) {
  return GetSWBuddyAllocator()->Used();
}
#endif
