...
template <typename T>
class SoftmaxWithCrossEntropySWKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    PADDLE_ENFORCE_EQ(
        platform::is_sw_place(context.GetPlace()), true,
        platform::errors::Unimplemented("This kernel only runs on SW."));
    const bool use_softmax = context.Attr<bool>("use_softmax");
    ...
    if (!use_softmax) {
      ...
      auto& dev_ctx =
          context.template device_context<platform::SWDeviceContext>();

      math::CrossEntropyFunctor<platform::SWDeviceContext, T>()(
          dev_ctx, &loss_2d, &softmax_2d, &labels_2d, soft_label,
          context.Attr<int>("ignore_index"), axis_dim);
    ...
    auto& dev_ctx =
        context.template device_context<platform::SWDeviceContext>();
    math::SoftmaxFunctor<platform::SWDeviceContext, T, false>()(
        dev_ctx, axis_dim, &logits_2d, &softmax_2d);
    math::CrossEntropyFunctor<platform::SWDeviceContext, T>()(
        dev_ctx, &loss_2d, &softmax_2d, &labels_2d, soft_label,
        context.Attr<int>("ignore_index"), axis_dim);
...
template <typename T>
class SoftmaxWithCrossEntropyGradSWKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    const Tensor* out_grad =
        context.Input<Tensor>(framework::GradVarName("Loss"));
    ...
    auto out_grad_mat = framework::EigenMatrix<T>::From(out_grad_2d);
    auto logit_grad_mat = framework::EigenMatrix<T>::From(logit_grad_2d);
    auto& place = *context.template device_context<platform::SWDeviceContext>()
                       .eigen_device();
    if (!use_softmax) {
      // use_softmax step1
      if (soft_label) {
