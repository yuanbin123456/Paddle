...
template <typename DeviceContext, typename T>
class MatMulSWKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &context) const override {
    auto &x = GET_DATA_SAFELY(context.Input<framework::Tensor>("X"), "Input",
                              "X", "MatMul");
...
#ifdef PADDLE_WITH_SW
template <typename DeviceContext, typename T>
class MatMulGradSWKernel : public framework::OpKernel<T> {
 public:
  void MatMul(const framework::ExecutionContext &context,
              const framework::Tensor &a, bool trans_a,
              const framework::Tensor &b, bool trans_b,
...
#ifdef PADDLE_WITH_SW
template <typename DeviceContext, typename T>
class MatMulDoubleGradSWKernel : public framework::OpKernel<T> {
 public:
  void MatMul(const framework::ExecutionContext &context,
              const framework::Tensor &a, bool trans_a,
              const framework::Tensor &b, bool trans_b, bool flag,
...
REGISTER_OP_SW_KERNEL(
    matmul, ops::MatMulSWKernel<paddle::platform::SWDeviceContext, float>,
    ops::MatMulSWKernel<paddle::platform::SWDeviceContext, double>);
REGISTER_OP_SW_KERNEL(
    matmul_grad,
    ops::MatMulGradSWKernel<paddle::platform::SWDeviceContext, float>,
    ops::MatMulGradSWKernel<paddle::platform::SWDeviceContext, double>);

REGISTER_OP_SW_KERNEL(
    matmul_grad_grad,
    ops::MatMulDoubleGradSWKernel<paddle::platform::SWDeviceContext, float>,
    ops::MatMulDoubleGradSWKernel<paddle::platform::SWDeviceContext, double>);
#endif
