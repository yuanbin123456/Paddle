 ...
 public:
  void Compute(const paddle::framework::ExecutionContext &ctx) const override {
    ...
    if (actual_place == -1) {
      ...
      else if(platform::is_sw_place(ctx.GetPlace())){
        actual_place = 4;
      }
    }
    ...
    if (actual_place == 0) {
      ...
      tensor->mutable_data(ctx.GetPlace(), data_type);
      math::SetConstant<platform::SWDeviceContext, T> functor;
      functor(reinterpret_cast<const platform::SWDeviceContext &>(dev_ctx),
              tensor, static_cast<T>(value));
#else
      PADDLE_THROW(platform::errors::PreconditionNotMet(
          "PaddlePaddle should compile with SW."));
#endif
    }
    else {
      PADDLE_THROW(platform::errors::Unimplemented(
