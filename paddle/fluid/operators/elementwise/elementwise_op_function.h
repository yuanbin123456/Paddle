...
template <typename T>
class RowwiseTransformIterator<T, platform::SWDeviceContext>
    : public std::iterator<std::random_access_iterator_tag, T, std::ptrdiff_t,
                           T *, T &> {
 public:
  RowwiseTransformIterator(const T *ptr, int n) : ptr_(ptr), i_(0), n_(n) {}

  RowwiseTransformIterator<T, platform::SWDeviceContext> &operator++() {
    ++i_;
    if (UNLIKELY(i_ == n_)) {
      i_ = 0;
    }
  ...
  RowwiseTransformIterator<T, platform::SWDeviceContext> &operator+(int n) {
    while (n-- > 0) {
      ++i_;
      if (UNLIKELY(i_ == n_)) {
        i_ = 0;
  ...
  bool operator==(const RowwiseTransformIterator<T, platform::SWDeviceContext>
                      &rhs) const {
    return (ptr_ + i_) == &(*rhs);
  }
...
template <typename T>
class MidWiseTransformIterator<T, platform::SWDeviceContext>
    : public std::iterator<std::random_access_iterator_tag, T, std::ptrdiff_t,
                           T *, T &> {
 public:
  MidWiseTransformIterator(const T *ptr, int n, int post)
  ...
  MidWiseTransformIterator<T, platform::SWDeviceContext> &operator++() {
    ++j_;
    if (UNLIKELY(j_ == post_)) {
      ++i_;
      j_ = 0;
  ...
  MidWiseTransformIterator<T, platform::SWDeviceContext> &operator+(int n) {
    while (n-- > 0) {
      ++j_;
      if (UNLIKELY(j_ == post_)) {
        ++i_;
  ...
  bool operator==(const MidWiseTransformIterator<T, platform::SWDeviceContext>
                      &rhs) const {
    return (ptr_ + i_) == &(*rhs);
  }
