...
template <typename T>
struct SameDimsElemwiseMul<
    platform::SWDeviceContext, T,
    typename std::enable_if<std::is_floating_point<T>::value>::type> {
  void operator()(const framework::ExecutionContext &ctx,
                  const framework::Tensor *x, const framework::Tensor *y,
                  framework::Tensor *z) {
    auto blas = math::GetBlas<platform::SWDeviceContext, T>(ctx);
    blas.VMUL(x->numel(), x->data<T>(), y->data<T>(), z->data<T>());
  }
};
template <typename T>
...
struct SameDimsElemwiseMul<
  ...
  void operator()(const framework::ExecutionContext &ctx,
    ...
    auto eigen_y = framework::EigenVector<T>::Flatten(*y);
    auto eigen_z = framework::EigenVector<T>::Flatten(*z);
    auto &place = *ctx.template device_context<platform::SWDeviceContext>()
                       .eigen_device();
    eigen_z.device(place) = eigen_x * eigen_y;
  }
};
...
REGISTER_OP_SW_KERNEL(
    elementwise_mul,
    ops::ElementwiseMulSWKernel<paddle::platform::SWDeviceContext, float>,
    ops::ElementwiseMulSWKernel<paddle::platform::SWDeviceContext, double>,
    ops::ElementwiseMulSWKernel<paddle::platform::SWDeviceContext, int>,
    ops::ElementwiseMulSWKernel<paddle::platform::SWDeviceContext, int64_t>,
    ops::ElementwiseMulSWKernel<paddle::platform::SWDeviceContext,
                              paddle::platform::complex64>,
    ops::ElementwiseMulSWKernel<paddle::platform::SWDeviceContext,
                              paddle::platform::complex128>);
REGISTER_OP_SW_KERNEL(
    elementwise_mul_grad,
    ops::ElementwiseMulGradSWKernel<paddle::platform::SWDeviceContext, float>,
    ops::ElementwiseMulGradSWKernel<paddle::platform::SWDeviceContext, double>,
    ops::ElementwiseMulGradSWKernel<paddle::platform::SWDeviceContext, int>,
    ops::ElementwiseMulGradSWKernel<paddle::platform::SWDeviceContext, int64_t>,
    ops::ElementwiseMulGradSWKernel<paddle::platform::SWDeviceContext,
                                  paddle::platform::complex64>,
    ops::ElementwiseMulGradSWKernel<paddle::platform::SWDeviceContext,
                                  paddle::platform::complex128>);
REGISTER_OP_SW_KERNEL(
    elementwise_mul_grad_grad,
    ops::ElementwiseMulDoubleGradSWKernel<paddle::platform::SWDeviceContext,
                                        float>,
    ops::ElementwiseMulDoubleGradSWKernel<paddle::platform::SWDeviceContext,
                                        double>,
    ops::ElementwiseMulDoubleGradSWKernel<paddle::platform::SWDeviceContext,
