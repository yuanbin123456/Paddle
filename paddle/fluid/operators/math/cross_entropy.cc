...
class CrossEntropyFunctor<platform::CPUDeviceContext, T> {
 public:
  void operator()(const platform::CPUDeviceContext& ctx, framework::Tensor* out,
    ...
    //cout<<"batch_size"<<batch_size<<"num_classes"<<num_classes<<"num_remain"<<num_remain<<endl;
...
template <typename T>
class CrossEntropyFunctor<platform::SWDeviceContext, T> {
 public:
  void operator()(const platform::SWDeviceContext& ctx, framework::Tensor* out,
                  const framework::Tensor* prob,
                  const framework::Tensor* labels, const bool softLabel,
                  const int ignore_index, const int axis_dim) {
    const int batch_size = prob->dims()[0];
...
template class CrossEntropyFunctor<platform::SWDeviceContext, float>;
template class CrossEntropyFunctor<platform::SWDeviceContext, double>;
#endif
}  // namespace math
}  // namespace operators
}  // namespace paddle
