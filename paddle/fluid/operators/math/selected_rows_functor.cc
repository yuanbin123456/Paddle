...
template <typename T>
struct SelectedRowsAdd<platform::SWDeviceContext, T> {
  void operator()(const platform::SWDeviceContext& context,
    ...
    PADDLE_ENFORCE_EQ(platform::is_sw_place(in1_place), true,
                      platform::errors::InvalidArgument(
                          "The running enviroment is not on the SW place."));
    auto in2_place = input2.place();
    PADDLE_ENFORCE_EQ(platform::is_sw_place(in2_place), true,
                      platform::errors::InvalidArgument(
                          "The running enviroment is not on the SW place."));
    auto out_place = context.GetPlace();
    PADDLE_ENFORCE_EQ(platform::is_sw_place(out_place), true,
                      platform::errors::InvalidArgument(
                          "The running enviroment is not on the SW place."));

    auto* out_data = out_value->data<T>();
    auto* in1_data = in1_value.data<T>();
    memory::Copy(BOOST_GET_CONST(platform::SWPlace, out_place), out_data,
                 BOOST_GET_CONST(platform::SWPlace, in1_place), in1_data,
                 in1_value.numel() * sizeof(T));

    auto* in2_data = in2_value.data<T>();
    memory::Copy(BOOST_GET_CONST(platform::SWPlace, out_place),
                 out_data + in1_value.numel(),
                 BOOST_GET_CONST(platform::SWPlace, in2_place), in2_data,
                 in2_value.numel() * sizeof(T));
  }
};

template struct SelectedRowsAdd<platform::SWDeviceContext, float>;
template struct SelectedRowsAdd<platform::SWDeviceContext, double>;

template <typename T>
struct SelectedRowsAddTensor<platform::SWDeviceContext, T> {
  void operator()(const platform::SWDeviceContext& context,
                  const framework::SelectedRows& input1,
    ...
    SetConstant<platform::SWDeviceContext, T> functor;
...
template struct SelectedRowsAddTensor<platform::SWDeviceContext, float>;
template struct SelectedRowsAddTensor<platform::SWDeviceContext, double>;

template <typename T>
struct SelectedRowsAddTo<platform::SWDeviceContext, T> {
  void operator()(const platform::SWDeviceContext& context,
                  const framework::SelectedRows& input1,
    ...
    auto* in1_data = in1_value.data<T>();
    auto* in2_data = in2_value->data<T>();
    memory::Copy(BOOST_GET_CONST(platform::SWPlace, in2_place),
...
template struct SelectedRowsAddTo<platform::SWDeviceContext, float>;
template struct SelectedRowsAddTo<platform::SWDeviceContext, double>;
template struct SelectedRowsAddTo<platform::SWDeviceContext, int>;
template struct SelectedRowsAddTo<platform::SWDeviceContext, int64_t>;

template <typename T>
struct SelectedRowsSumTo<platform::SWDeviceContext, T> {
  void operator()(const platform::SWDeviceContext& context,
    ...
    auto* in2_value = input2->mutable_value();
    auto* in2_data = in2_value->data<T>();
    auto blas = math::GetBlas<platform::SWDeviceContext, T>(context);
    size_t offset = 0u;
...
template struct SelectedRowsSumTo<platform::SWDeviceContext, float>;
template struct SelectedRowsSumTo<platform::SWDeviceContext, double>;

template <typename T>
struct SelectedRowsAddToTensor<platform::SWDeviceContext, T> {
  void operator()(const platform::SWDeviceContext& context,
                  const framework::SelectedRows& input1,
...
template struct SelectedRowsAddToTensor<platform::SWDeviceContext, float>;
template struct SelectedRowsAddToTensor<platform::SWDeviceContext, double>;
template struct SelectedRowsAddToTensor<platform::SWDeviceContext, int>;
template struct SelectedRowsAddToTensor<platform::SWDeviceContext, int64_t>;
template struct SelectedRowsAddToTensor<platform::SWDeviceContext,
                                        platform::bfloat16>;
...
template <typename T>
typename std::enable_if<std::is_floating_point<T>::value>::type
elementwise_add_to(BlasT<platform::SWDeviceContext, T>* blas, size_t data_len,
                   const T* in, T* out) {
  blas->AXPY(data_len, T(1.f), in, out);
}
...
template <typename T>
typename std::enable_if<std::is_integral<T>::value>::type elementwise_add_to(
    BlasT<platform::SWDeviceContext, T>* blas, size_t data_len, const T* in,
    T* out) {
  for (size_t i = 0; i < data_len; i++) {
    out[i] += in[i];
  }
...
template <typename T>
struct MergeAdd<platform::SWDeviceContext, T> {
  framework::SelectedRows operator()(const platform::SWDeviceContext& context,
                                     const framework::SelectedRows& input,
                                     const bool sorted_result = false) {
    framework::SelectedRows out;
    (*this)(context, input, &out, sorted_result);
  ...
  void operator()(const platform::SWDeviceContext& context,
    ...
    if (merged_row_set.size() == row_num && !sorted_result) {
      ...
      for (auto* in : inputs) {
        ...
        memory::Copy(BOOST_GET_CONST(platform::SWPlace, out_place),
                     out_data + copied_numel,
                     BOOST_GET_CONST(platform::SWPlace, in_place), in_data,
                     in_numel * sizeof(T));
        copied_numel += in_numel;
      }
    } else {
      ...
      math::SetConstant<platform::SWDeviceContext, T> constant_functor;
      constant_functor(context, out.mutable_value(), static_cast<T>(0.f));
...
template <typename T>
struct MergeAverage<platform::SWDeviceContext, T> {
  framework::SelectedRows operator()(const platform::SWDeviceContext& context,
                                     const framework::SelectedRows& input) {
    framework::SelectedRows out;
    (*this)(context, input, &out);
...
template struct MergeAdd<platform::SWDeviceContext, int>;
template struct MergeAdd<platform::SWDeviceContext, int64_t>;
template struct MergeAdd<platform::SWDeviceContext, float>;
template struct MergeAdd<platform::SWDeviceContext, double>;
template struct MergeAdd<platform::SWDeviceContext,
                         paddle::platform::complex64>;
template struct MergeAdd<platform::SWDeviceContext,
                         paddle::platform::complex128>;
template struct MergeAdd<platform::SWDeviceContext,
...
template struct MergeAverage<platform::SWDeviceContext, int>;
template struct MergeAverage<platform::SWDeviceContext, int64_t>;
template struct MergeAverage<platform::SWDeviceContext, float>;
template struct MergeAverage<platform::SWDeviceContext, double>;

template <typename T>
struct UpdateToTensor<platform::SWDeviceContext, T> {
  void operator()(const platform::SWDeviceContext& context,
                  const ScatterOps& op, const framework::SelectedRows& input1,
                  framework::Tensor* input2) {
    auto in1_height = input1.height();
