...
template <class DeviceContext>
using enable_if_SW = typename std::enable_if<
    std::is_same<DeviceContext, platform::SWDeviceContext>::value>::type;

template <typename DeviceContext, typename T, bool is_test>
class SoftmaxFunctor<DeviceContext, T, is_test, enable_if_SW<DeviceContext>> {
 public:
  void operator()(const DeviceContext& context, const int axis_dim,
                  const framework::Tensor* X, framework::Tensor* Y) {
    auto in_dims = X->dims();
...
template <typename DeviceContext>
class SoftmaxFunctor<DeviceContext, float, true, enable_if_SW<DeviceContext>> {
 public:
  void operator()(const DeviceContext& context, const int axis_dim,
                  const framework::Tensor* X, framework::Tensor* Y) {
    auto in_dims = X->dims();
    ...
    // 2D data. Batch x C
    auto compute_softmax =
        jit::KernelFuncs<jit::SoftmaxTuple<float>, platform::SWPlace>::Cache()
            .At(in_dims[kClassDim]);
    compute_softmax(in_data, out_data, in_dims[kClassDim], in_dims[kBatchDim],
                    in_dims[kClassDim] / axis_dim);
  }
...
#ifdef PADDLE_WITH_SW
template <typename DeviceContext, typename T>
class SoftmaxGradFunctor<DeviceContext, T, enable_if_SW<DeviceContext>> {
 public:
  void operator()(const DeviceContext& context, const int axis_dim,
                  const framework::Tensor* y, const framework::Tensor* y_grad,
                  framework::Tensor* x_grad) {
