...
platform::DeviceContext* DeviceContextPool::Get(const platform::Place& place) {
  ...
  if (it == device_contexts_.end()) {//==
    PADDLE_THROW(platform::errors::Unimplemented(
        ...
        "with WITH_GPU, WITH_XPU or WITH_ASCEND_CL/WITH_SW option or check that "
...
#endif
    } else if(platform::is_sw_place(p)){
#ifdef PADDLE_WITH_SW
       EmplaceDeviceContext<SWDeviceContext, SWPlace>(&device_contexts_, p);
#else
      PADDLE_THROW(
          platform::errors::Unimplemented("SWPlace is not supported. Please "
                                          "re-compile with WITH_SW option."));
#endif
...
#ifdef PADDLE_WITH_SW
SWDeviceContext::SWDeviceContext(){
  eigen_device_.reset(new Eigen::DefaultDevice());
}
SWDeviceContext::SWDeviceContext(SWPlace place) : place_(place) {
  eigen_device_.reset(new Eigen::DefaultDevice());
}

Eigen::DefaultDevice* SWDeviceContext::eigen_device() const {
  return eigen_device_.get();
}

Place SWDeviceContext::GetPlace() const { return place_; }
#endif
#ifdef PADDLE_WITH_XPU
