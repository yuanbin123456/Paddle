...
struct SWPlace{
  SWPlace(){}
  // needed for variant equality comparison
  inline bool operator==(const SWPlace &) const { return true; }
  inline bool operator!=(const SWPlace &) const { return false; }
  inline bool operator<(const SWPlace &) const { return false; }
};
...
struct IsCUDAPlace : public boost::static_visitor<bool> {
  ...
  bool operator()(const SWPlace &) const { return false; }
};
...
struct IsSWPlace : public boost::static_visitor<bool>{
  bool operator()(const CPUPlace &) const { return false; }
  bool operator()(const XPUPlace &) const { return false; }
  bool operator()(const NPUPlace &) const { return false; }
  bool operator()(const CUDAPlace &) const { return false; }
  bool operator()(const CUDAPinnedPlace &) const { return false; }
  bool operator()(const SWPlace &) const { return true; }
};
...
class Place : public boost::variant<CUDAPlace, XPUPlace, NPUPlace, CPUPlace,
                                    CUDAPinnedPlace, SWPlace> {
 private:
  using PlaceBase =
      boost::variant<CUDAPlace, XPUPlace, NPUPlace, CPUPlace, CUDAPinnedPlace, SWPlace>;
 ...
 public:
  ...
  Place(const SWPlace &sw_place) : PlaceBase(sw_place){}
...
struct PlaceVisitorWrapper
  ...
  typename Visitor::result_type operator()(const SWPlace &sw) const {
#ifdef PADDLE_WITH_SW
    return visitor_(sw);
#else
    PADDLE_THROW(platform::errors::Unavailable(
        "Paddle is not compiled with SW. Cannot visit SW device"));
    return typename Visitor::result_type();
#endif
  }
