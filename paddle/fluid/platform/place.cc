...
class PlacePrinter : public boost::static_visitor<> {
 public:
  ...
  void operator()(const SWPlace &p){ os_ << "SWPlace";}
 private:
  std::ostream &os_;
};
...
bool is_sw_place(const Place &p){
  return boost::apply_visitor(IsSWPlace(), p);
}
...
bool is_same_place(const Place &p1, const Place &p2) {
  if (places_are_same_class(p1, p2)) {
    ...
    }else if (is_sw_place(p1)){
      return BOOST_GET_CONST(SWPlace, p1) == BOOST_GET_CONST(SWPlace, p2);
    } else {
