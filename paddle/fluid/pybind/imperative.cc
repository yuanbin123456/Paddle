...
static const platform::Place PyObjectToPlace(const py::object &place_obj) {
  ...
  } else if (py::isinstance<platform::SWPlace>(place_obj)) {
    return place_obj.cast<platform::SWPlace>();
  } else if (py::isinstance<platform::Place>(place_obj)) {
    return place_obj.cast<platform::Place>();
  } else {
    PADDLE_THROW(platform::errors::InvalidArgument(
        "Place should be one of "
        "Place/CPUPlace/XPUPlace/CUDAPlace/CUDAPinnedPlace/SWPlace"));
  }
}
...
static void InitTensorForVarBase(imperative::VarBase *self,
  ...
  }  else if (platform::is_sw_place(place)) {
    SetTensorFromPyArray<platform::SWPlace>(
        tensor, array, BOOST_GET_CONST(platform::SWPlace, place), zero_copy);
...
void BindImperative(py::module *m_ptr) {
  ...
  py::class_<imperative::VarBase, std::shared_ptr<imperative::VarBase>>(
      ...
      .def("__init__", &InitVarBaseFromNumpyWithArg<platform::SWPlace>,
           py::arg("value"), py::arg("place"), py::arg("persistable") = false,
           py::arg("zero_copy") = false, py::arg("name") = "",
           py::arg("stop_gradient") = -1)
...
#undef TENSOR_TO_PY_SCALAR
      ...
      .def("cpu",
           [](const std::shared_ptr<imperative::VarBase> &self) {
            ...
            R"DOC(
        Returns a copy of this Tensor in SW memory.

        If this Tensor is already in SW memory, then no copy is performed and the original Tensor is returned.
        ...
        Examples:
            .. code-block:: python
              ...
              y = x.cpu()
              print(y.place)    # SWPlace

              )DOC")
      .def("sw",
           [](const std::shared_ptr<imperative::VarBase> &self) {
             if (platform::is_sw_place(self->Place())) {
               return self;
             } else {
               auto new_var = self->NewVarBase(platform::SWPlace(), true);
               new_var->SetOverridedStopGradient(self->OverridedStopGradient());
               return new_var;
             }
           },
  ...
  py::class_<imperative::Tracer, std::shared_ptr<imperative::Tracer>>(
      ...
      .def_property(
          ...
          [](imperative::Tracer &self, const py::object &obj) {
            if (py::isinstance<platform::CUDAPlace>(obj)) {
              ...
              VLOG(4) << "Tracer(" << &self << ")"
                      << " set expected place " << *p;
            } else if (py::isinstance<platform::SWPlace>(obj)) {
              auto p = obj.cast<platform::SWPlace *>();
              self.SetExpectedPlace(*p);
              VLOG(4) << "Tracer(" << &self << ")"
                      << " set expected place " << *p;
      ...
      .def("trace",
           [](imperative::Tracer &self, const std::string &type,
              const PyNameVarBaseMap &ins, const PyNameVarBaseMap &outs,
              framework::AttributeMap attrs, const platform::SWPlace &place,
              bool trace_backward) {
             auto ins_map = ConvertToNameVarBaseMap(ins);
             auto outs_map = ConvertToNameVarBaseMap(outs);
             {
  ...
  m.def("pylayer_apply",
        [](const platform::SWPlace &place, const py::object &cls,
           const py::args args, const py::kwargs kwargs) {
          return imperative::PyLayerApply(place, cls, args, kwargs);
        });
