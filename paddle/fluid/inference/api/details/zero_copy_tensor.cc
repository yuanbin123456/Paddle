...
T *Tensor::mutable_data(PlaceType place) {
  ...
  switch (static_cast<int>(place)) {
    ...
    case static_cast<int>(PlaceType::kSW): {
      return tensor->mutable_data<T>(paddle::platform::SWPlace());
    }
    default:
...
T *Tensor::data(PlaceType *place, int *size) const {
  ...
  } else if (paddle::platform::is_sw_place(tensor->place())) {
    *place = PlaceType::kSW;
  } else {
...
void Tensor::CopyFromCpu(const T *data) {
  ...
  if (place_ == PlaceType::kCPU) {
    ...
    PADDLE_THROW(paddle::platform::errors::Unavailable(
        ...
        "with CUDA."));
#endif
  } else if(place_ == PlaceType::kSW){
#ifdef PADDLE_WITH_SW
    auto *t_data = tensor->mutable_data<T>(paddle::platform::SWPlace());
    std::memcpy(static_cast<void *>(t_data), data, ele_size);
#else
    PADDLE_THROW(paddle::platform::errors::Unavailable(
        "Can not create tensor with SW place because paddle is not compiled "
        "with SW."));
#endif
